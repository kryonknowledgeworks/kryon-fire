"""This module contains the practitioner resource."""
...
class Practitioner(DomainResource):
    """This class validates the practitioner resource"""

    def __init__(self, resource):
        self.validation_report = None
        print("From ===> class Practitioner")
        super().__init__(resource)
        self.resource = resource
        self.schema = get_resource_schema("practitioner")
        self.do_validate()

    def do_validate(self):
        """This method validates the resource"""
        for key in self.schema.keys():
            if (self.schema[key].get('cardinality') == '1..1'
                    and not self.resource.get(key)):
